...
import TestQueue from "./components/TestQueue/TestQueue";
import PluginContext from "./context/PluginContext";

export function App() {
  const { pluginState, updateApiListeners } = usePlugin({
  ...
  const contextValue = {
    ...
    updateApiListeners: updateApiListeners,
  };
  const pluginContent =
    pluginState.unprocessedTests.length > 0 ? (
      <TestQueue pluginState={pluginState} />
    ) : (
      <HomePage pluginState={pluginState} />
    );
